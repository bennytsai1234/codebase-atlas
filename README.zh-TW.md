...
```text
請使用 Codebase Atlas 技能處理 reader 程式碼倉庫。
```

程式代理不應該立刻開始完整掃描。它應該先簡短說明：Codebase Atlas 會掃描目標倉庫，並在 `docs/` 下建立可長期使用的 Markdown 文檔，包含模組索引、各模組責任與依賴、影響範圍、已知風險、常見修改入口，以及後續工作流文檔。

接著先詢問：

1. 產出要使用英文還是繁體中文？
2. 這次要使用獨立專案模式，還是參考資料輔助模式？如果是參考資料輔助模式，參考資料的路徑、URL 或檔案是什麼？
3. 後續每次依照工作流完成工作後，要不提交、只提交，還是提交並推送？
4. 如果是參考資料輔助模式，是否啟用 feature parity workflow？預設不啟用；只有目標是功能對齊、相容性、遷移等價，或依參考資料擴充功能時才啟用。
...
feature parity workflow 預設不啟用。參考資料輔助模式下，程式代理應該先詢問是否啟用；只有你明確要求功能對齊、相容性、遷移等價，或依參考資料擴充功能時，才會產生功能對齊用的工作流文檔。
...
技能應該先說明它會產生什麼，並在完整掃描前詢問產出語言、Atlas 模式、交付政策，以及是否啟用 feature parity workflow。如果選擇「參考資料輔助模式」，請在程式代理開始完整掃描前提供參考資料的路徑、URL 或檔案。

...
- 此模組提供生成 index、module docs 與可選 workflow entrypoint adapters 的起始模板。
...
- `assets/templates/codex_skill_workflow.md`
- `assets/templates/vscode_prompt_workflow.prompt.md`

## 上游依賴

- `references/doc-output-contract.md` 定義 index document、standalone module document、reference-assisted module document 與 workflow entry adapters 必填內容。
...
- Generated index 是否包含 workflow links、workflow entrypoints、module links、routing-oriented summaries 與 delivery policy，取決於 `index.md`。
...
- Generated adapters 是否保持薄導流，取決於 entrypoint templates 與 contract。

## 主要流程

- 初始化時，代理人複製或依照模板建立 index、module docs 與 workflow docs。
- 若使用者要求工具入口，代理人依 adapter templates 產生 Codex skills 或 prompt files。
- Placeholder 名稱包含 `ATLAS_TITLE`、`DELIVERY_POLICY`、`WORKFLOW_LINKS`、`MODULE_LINKS`、`MODULE_SUMMARIES` 等，必須被具體內容替換。
- Adapter placeholder 包含 `WORKFLOW_ENTRYPOINT_NAME`、`WORKFLOW_ENTRYPOINT_TITLE`、`WORKFLOW_ENTRYPOINT_DESCRIPTION` 與 `WORKFLOW_FILE`。
...
- 想改 Codex skill adapter：從 `assets/templates/codex_skill_workflow.md` 開始。
- 想改 prompt-file adapter：從 `assets/templates/vscode_prompt_workflow.prompt.md` 開始。
...
- 新增 adapter placeholder 時，必須同步 skeleton prompt 與 final validation，避免工具入口殘留模板標記。
...
- Adapter template 若變成第二套 workflow，未來維護會在 `docs/` 與工具入口之間分裂。
...
- 不要讓 adapter 取代 canonical `docs/` workflow。

...
- 此模組定義 Codebase Atlas 生成文檔與可選 workflow entry adapters 時必須遵循的輸出契約與 skeleton 建立流程。
...
- 可選 adapter templates：`assets/templates/codex_skill_workflow.md`、`assets/templates/vscode_prompt_workflow.prompt.md`
...
- `docs/` 產出的檔名、目錄結構、連結、workflow 清單、module section、workflow entry adapters 與 delivery policy 都受此模組約束。
- README 的輸出結構說明應與契約一致。
- 模板中的 placeholder 必須能被 skeleton 流程完整替換。

## 主要流程

- `doc-output-contract.md` 定義初始決策、命名、source of truth、語言、交付政策、workflow entry adapters、掃描邊界、standalone/reference-assisted output 與 final validation。
- `create-doc-skeleton-prompt.md` 將契約轉成實作流程，要求先掃描 target repo，再選模式指南、填模板、產生可選薄 adapter、檢查 placeholder 與執行品質檢查。
...
- 想改工具入口：從 `Workflow Entry Adapters` 與 `Index Document` 開始。
...
- 改 workflow entry adapter 時，先更新 `doc-output-contract.md`，再同步 skeleton prompt、adapter templates、README 與自舉 index。
...
- Adapter 若複製完整 workflow body，會和 canonical `docs/` workflow 分裂；契約要求它們只能薄導流。
...
- 不要讓工具入口取代 `docs/` workflow 或成為第二份 canonical workflow。

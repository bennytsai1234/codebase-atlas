...
- 此模組也包含可選 workflow entrypoint adapter 模板；adapter 只負責把 Codex 或 IDE 入口導回 canonical `docs/` workflow。
...
- `references/create-doc-skeleton-prompt.md` 定義何時產生 Codex skill 或 prompt-file adapter。

## 下游影響

- 所有 generated workflow docs 都會從這些模板衍生。
- 如果使用者要求命令或選單呼叫，generated adapters 會從 entrypoint templates 衍生，但仍導回 `docs/` workflow。
...
- Adapter 保留使用者當前 request，開啟 canonical workflow，並要求從 index 開始；它不能複製完整 workflow body。
...
- 想改 Codex/IDE 入口：從 `assets/templates/codex_skill_workflow.md` 或 `assets/templates/vscode_prompt_workflow.prompt.md` 開始。
...
- 改工具入口時，同步 `references/doc-output-contract.md`、`references/create-doc-skeleton-prompt.md`、README 與自舉 index。
...
- Adapter 模板若寫太多實際 workflow 規則，會造成工具入口與 canonical workflow 漂移。
...
- 不要把完整 workflow body 複製進 Codex/IDE adapter。
